...
# Add stocks/ to path for shared feature engineering helpers, and repo root
# for the `shared` package that kalshi/db.py depends on.
_KALSHI_DIR = Path(__file__).parent
sys.path.insert(0, str(_KALSHI_DIR.parent / "stocks"))
...
ndef contract_features(current_price: float, strike: float,
...
def auto_place_trade(recommendation: dict, kalshi_client: KalshiClient, bucket: str) -> bool:
    ...
    try:
        ...
        # Check for resting (unfilled) limit orders — a pending order won't appear
        # in get_positions() yet, so without this check repeated scan cycles would
        # keep stacking duplicate orders on the same contract.
        try:
            open_orders = kalshi_client.get_open_orders(ticker=ticker)
            for order in open_orders:
                order_side = order.get("side", "").lower()
                order_action = order.get("action", "").lower()
                if order_action == "buy" and order_side == side:
                    logger.info(f"Resting buy order already exists: {ticker} {side.upper()} "
                                f"(order {order.get('order_id')}). Skipping.")
                    return False
        except Exception as e:
            logger.warning(f"Could not check open orders: {e}. Proceeding with caution.")
...
def sync_paper_trades_with_kalshi_fills(kalshi_client: KalshiClient) -> dict:
    ...
    try:
        ...
        fills = kalshi_client.get_fills(limit=100, min_ts=_since_ts)
